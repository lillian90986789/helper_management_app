...
-- 女佣每周食材申请（周五前提交，雇主 review 后分流：加入采购清单 / 雇主线上购买 / 拒绝）
CREATE TABLE IF NOT EXISTS MaidRequest (
  request_id INTEGER PRIMARY KEY AUTOINCREMENT,
  family_id INTEGER,
  maid_id INTEGER,
  week_start TEXT,                 -- 所在周的周一 YYYY-MM-DD
  name TEXT,
  name_en TEXT,
  quantity REAL DEFAULT 1,
  unit TEXT,
  notes TEXT,
  status TEXT DEFAULT 'pending',   -- pending | to_list | online | rejected
  shopping_item_id INTEGER,        -- 批准加入清单后对应的商品
  created_at TEXT DEFAULT (datetime('now','localtime')),
  reviewed_at TEXT
);
...
// Recipe → 喜欢程度打分 0–5 星（0=未打分；独立于 favorite 收藏）
addCol('Recipe', 'rating', 'INTEGER DEFAULT 0');
...
// 任务照片 AI 检查（如"洗衣机洗的衣服里不能有内衣内裤"）
addCol('TaskTemplate', 'photo_check_rule', 'TEXT');                  // 模板级规则，生成每日任务时快照
addCol('DailyTask', 'photo_check_rule', 'TEXT');                     // 当日任务规则快照（临时任务直接填）
addCol('DailyTaskAttachment', 'check_result', 'TEXT');               // 完成照片的检查结果 JSON {pass,reason}
...
// ShoppingList → 清单类型：family=家庭采购 / maid=女佣食材（每周申请流程生成）；账单 review 分开展示
addCol('ShoppingList', 'list_type', "TEXT DEFAULT 'family'");
// 存量数据按标题回填女佣类清单（幂等）
db.prepare("UPDATE ShoppingList SET list_type='maid' WHERE title LIKE '女佣食材申请%' AND (list_type IS NULL OR list_type='family')").run();

// ---- 采购历史归档：清单确认后快照已购商品（清单被彻底删除后记录仍保留）----
// 用途：①新建商品时按同名商品最近一次购买价自动填预算 ②账单 review 按月/分类/清单类型汇总
db.exec(`
CREATE TABLE IF NOT EXISTS PurchaseRecord (
  record_id INTEGER PRIMARY KEY AUTOINCREMENT,
  family_id INTEGER NOT NULL,
  shopping_list_id INTEGER,
  list_type TEXT DEFAULT 'family',          -- family|maid
  list_title TEXT,
  store_name TEXT,
...
CREATE INDEX IF NOT EXISTS idx_pr_family_name ON PurchaseRecord(family_id, name);
CREATE INDEX IF NOT EXISTS idx_pr_family_date ON PurchaseRecord(family_id, purchase_date);
`);
