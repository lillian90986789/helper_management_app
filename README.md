...
| 角色 | 方式 | 入口 | 凭据存储 |
|---|---|---|---|
| **雇主** | 用户名 + 密码 注册 / 登录 | `/#/register`（或 `/#/login`） | SQLite `User` 表，`username` + `password_hash`（sha256 加盐，非明文） |
| **女佣** | 邀请码加入 | `/#/join`（雇主在「女佣管理」页生成邀请码） | 加入即建女佣账号，身份记于浏览器 `localStorage` |

- 雇主注册字段：用户名（≥3 位）、密码（≥6 位）、称呼（女佣端可见，可选）、家庭名称（可选）。
- 女佣加入字段：邀请码 + 姓名 + 语言；演示邀请码 `HOME-8821`。
- 姓名 / 头像 / 家庭名称均可在「我的 → ✏️ 编辑资料」中修改，首页与女佣端同步。
- 也内置了 Google（Gmail）登录后端接口（配 `GOOGLE_CLIENT_ID` 启用，前端按钮待接）；旧的手机号/邮箱验证码注册向导保留在 `/#/register-wizard`。
- 当前为**单家庭**演示结构（后端按唯一家庭取数据）；如需「每个雇主独立家庭」的多租户隔离，需要一次后端改造。
