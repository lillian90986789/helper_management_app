...
export const api = {
  ...
  trashLists: () => req('/shopping/trash'),
  maidRequests: (offset = 0) => req('/maid-requests' + (offset ? `?offset=${offset}` : '')),
  addMaidRequest: (body) => req('/maid-requests', { method: 'POST', body }),
  deleteMaidRequest: (id) => req(`/maid-requests/${id}`, { method: 'DELETE' }),
  reviewMaidRequest: (id, body) => req(`/maid-requests/${id}/review`, { method: 'POST', body }),
  restoreList: (id) => req(`/shopping/${id}/restore`, { method: 'POST' }),
